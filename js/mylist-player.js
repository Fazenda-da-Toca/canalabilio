	...
	$.ajax({
	  ...
	  success: function(data) {
			...
			vid_frame[m] = new YT.Player('vid_frame'+m, {
		      height: '200',
		      width: '200',
		      playerVars: { 'html5': 1, 'showinfo': showVideoInfo, 'autoplay': 1, 'rel': showRelatedVideos, 'controls': showPlayerControls, 'playsinline': 1 },
		      videoId: vidIDs[n][0],
		      events: {
		    ...
		    // // função de pausar o timer que esta no javascript.js *PB
		    // pauseTimer();
	
			// set overall width of vList div
		    list_width[m] = listLength[m]*168;
		    $("#player-container"+m+">div.mlvp-list-container>div.mlvp-list").width(list_width[m]+"2px");
	    ...
	    if(event.data === 0){  // if video has ended
	        ...
	        if (nextVid >= listLength[y]) {
				...
				function getOffset( el ) {
		    	    ...
		    	    // função para voltar para a pagina anterior que esta no javascript.js *PB
					voltar();
...
function setScrollAmt(n) {
	...
	if (w<=420 && w>=316) { // smaller arrows on small screens
		...
		jQuery('.mlvp-arrows').css({ display: "block" });
		jQuery('.mlvp-list-container').css({ "margin-left": "50px", "margin-right": "50px" });
		jQuery('.demo-icon').css({ "font-size": "60px", "line-height": "1em" });
	}
